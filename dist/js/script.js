import { peliculas } from "../bbdd/basededatos.js";


// variables
let openCloseX = false; // Esta variable se utiliza para poner display a las X de las tarjetas de peliculas.
let openCloseModal = false // Esta variable se utiliza para poner display al modal del menu options.
let openCloseSearch = false // Esta variable se utiliza para poner display al input del boton search.

// Guardar colección en localstorage
localStorage.mitoppelis = localStorage.mitoppelis || JSON.stringify(peliculas);

//SELECTORES
let menuButton = document.getElementById("options")
let resetButton = document.getElementById("reset") 

let close = document.getElementsByClassName("topmovies__movielist__close");
...
//EVENTOS
/* Este asignada un evento al editbutton para que cambie el estilo display 
del botton de "eliminar" que tiene cada una de las cajas.*/
  ...
  /* Evento que cambia el style.display de un modal para abrir un menu
  donde habrá una option de resetear el localstorage.
  */
...
const resetTopMovies = ()=>{
    localStorage.clear();
    inicio()
} 
 
const eliminarDetop = (id) => {
  const listaStorage = JSON.parse(localStorage.mitoppelis);
  const nuevalista = listaStorage.filter((e) => listaStorage.indexOf(e) != id+1);
  console.log(nuevalista);
  localStorage.mitoppelis = JSON.stringify(nuevalista);
  inicio(); 
};

const inicio = () => {
  localStorage.mitoppelis = localStorage.mitoppelis || JSON.stringify(peliculas);
  const mispeliculas = JSON.parse(localStorage.mitoppelis);
  const divMovieList = document.getElementById("movielist");
  poster.style.backgroundImage = `url(${mispeliculas[0].imagen})`;
  divMovieList.innerHTML = moviesList(mispeliculas);
